...
type NavLabelKey = "home" | "movementsShort" | "netWorth" | "family";
...
const NAV_ITEMS: {
  ...
  { href: "/", labelKey: "home", icon: Home, match: (path: string) => path === "/" },
  { href: "/gastos", labelKey: "movementsShort", icon: WalletCards, match: (path: string) => path.startsWith("/gastos") },
  {
    href: "/patrimonio",
    labelKey: "netWorth",
    icon: Landmark,
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className="leading-none">{dictionary.nav[item.labelKey]}</span>
            </Link>
          );
        })}
      </div>
