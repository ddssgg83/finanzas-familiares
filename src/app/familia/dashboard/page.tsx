  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (goalsError) {
          console.warn("Error cargando metas (se asumirá sin metas):", goalsError.message);
        }
  ...
  const goalsWithProgress: GoalWithProgress[] = useMemo(() => {
    ...
    return goals.map((goal) => {
      ...
      if (target > 0 && totalContributed < target && last30Amount > 0) {
        ...
        projectedDaysToGoal = (target - totalContributed) / dailyVelocity;
      }
      ...
      if (goal.due_date) {
        ...
        if (progressPct >= 100) intensity = "on_track";
        else if (projectedDaysToGoal === null) intensity = "at_risk";
        else if (projectedDaysToGoal > daysToDue) intensity = "off_track";
  ...
  const globalSummary = useMemo(() => {
    ...
    const monthlyGoalContrib = txs
      .filter((tx) => {
        ...
        return d >= startOfMonth && d <= now && tx.goal_id != null;
      })
      .reduce((sum, tx) => sum + (tx.amount || 0), 0);
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                  <div
                    ...
                    style={{ width: `${Math.min(globalSummary.globalProgressPct, 100)}%` }}
                  />
                </div>
                ...
                {goalsWithProgress.length === 0 ? (
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {goalsWithProgress.map((goal) => {
                      const member = goal.owner_user_id ? memberMap.get(goal.owner_user_id) : null;
